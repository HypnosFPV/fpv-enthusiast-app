...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function thumbnailUri(post: any): string | null {
  if (post.thumbnail_url) return post.thumbnail_url;
  if (post.media_url && !post.media_url.match(/\.(mp4|mov|webm)$/i)) {
    return post.media_url;
  }
  const ytSrc = post.source_url || post.embed_url || post.media_url || '';
  if (ytSrc) {
    const m = ytSrc.match(
...
export default function ProfileScreen() {
  ...
  const {
    linked: ytLinked,
    loading: ytAuthLoading,
    promptAsync: promptYouTubeAuth,
    unlinkYouTube,
  ...
  // Seed edit forms when profile loads
  useEffect(() => {
    if (!profile) return;
...
const loadMyPosts = useCallback(async () => {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([fetchProfile(), loadTabData(activeTab)]);
    setRefreshing(false);
  }, [fetchProfile, loadTabData, activeTab]);
  ...
  const renderGridCell = useCallback(({ item }: { item: Post }) => {
    ...
    const isYT  = !!(item.source_url || item.embed_url || item.media_url || '')
                    .match(/youtu/);
    const isIG  = item.source_platform === 'instagram' ||
      ...
      >
        ...
        {thumb ? (
          ...
          <View style={[styles.gridThumb, styles.gridThumbPlaceholder]}>
            <Ionicons
              name={isVid ? 'videocam' : 'image-outline'}
              size={28}
              color="#444"
            />
          </View>
        ...
        {/* Video play badge (non-YouTube videos) */}
        {isVid && !isYT && (
  ...
  return (
    <View style={styles.root}>
      ...
      >
        {/* ── Banner ── */}
        <TouchableOpacity onPress={uploadHeaderImage} activeOpacity={0.85}>
        ...
        <View style={styles.bioSection}>
          ...
          <View style={styles.socialRow}>
            ...
            <TouchableOpacity
              ...
              onPress={() => setShowSocialLinks(true)}
            >
          ...
          <>
            ...
            {activeTab === 'builds' && (
              <View>
                ...
                <TouchableOpacity
                  style={styles.fab}
                  onPress={() => setShowCreateBuild(true)}
                >
              ...
              <View style={styles.settingsRow}>
                ...
                <Switch
                  ...
                  onValueChange={(val: boolean) => {
                    void updateProfile({ autoplay_videos: val });
                  }}
            ...
            <View style={styles.settingsSection}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setTimeout(() => setShowSocialLinks(true), 350);
                }}
              >
              ...
              <View style={[
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  ...
                  <View>
                    ...
                    <Text style={[
                      ...
                      {ytLinked
                        ? '● Connected — Like & Subscribe enabled'
                        : '○ Not connected'}
                    </Text>
                  </View>
                </View>
                <TouchableOpacity
                  style={[
                    styles.ytAuthBtn,
                    ytLinked ? styles.ytAuthBtnUnlink : styles.ytAuthBtnLink,
                  ]}
        ...
        >
          ...
          <ScrollView contentContainerStyle={styles.modalBody}>
            ...
            <TouchableOpacity
              style={[
                styles.primaryBtn,
                updating ? styles.primaryBtnDisabled : undefined,
              ]}
              onPress={saveProfile}
              disabled={updating}
            >
      ...
      <Modal
        ...
        animationType="slide"
        presentationStyle="overFullScreen"
        onRequestClose={() => setShowSocialLinks(false)}
      >
        <KeyboardAvoidingView
          style={styles.modalRoot}
          behavior={Platform.OS === 'ios' ? 'padding' : 'height'}
      ...
      {/* ── Create Build Modal ── */}
      <Modal
        visible={showCreateBuild}
        animationType="slide"
        presentationStyle="overFullScreen"
        onRequestClose={() => setShowCreateBuild(false)}
      >
        <KeyboardAvoidingView
          style={styles.modalRoot}
          behavior={Platform.OS === 'ios' ? 'padding' : 'height'}
...
const styles = StyleSheet.create({
  ...
  banner:            { width: '100%', height: 160, backgroundColor: '#111' },
  ...
  settingsSection:      { backgroundColor: '#1a1a2e', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: '#2a2a4a' },
  ...
  signOutBtn:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1a1a2e', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: '#e74c3c33', gap: 8 },
  signOutText: { color: '#e74c3c', fontWeight: '700', fontSize: 15 },

  inputLabel:     { color: '#aaa', fontSize: 12, fontWeight: '600', marginBottom: 4 },
  input:          { backgroundColor: '#1e1e3a', borderRadius: 10, padding: 12, color: '#fff', fontSize: 14, borderWidth: 1, borderColor: '#2a2a4a' },
